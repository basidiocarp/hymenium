...
mod tests {
    ...
    mod splitting {
        ...
        fn split_at_effort_boundary() {
            ...
            let config = DecompositionConfig {
                max_effort_per_piece_secs: 4 * 3600,
                ..Default::default()
            };
    ...
    mod tier_assignment {
        ...
        fn tier_from_effort() {
            ...
            let mut s = make_step(1, "Quick");
            s.effort = Some("30 minutes".to_string());
            let result = decompose(&make_handoff("T", vec![s]), &config).unwrap();
            assert_eq!(result.pieces[0].suggested_tier, AgentTier::Haiku);

            let mut s = make_step(1, "Medium");
            s.effort = Some("3 hours".to_string());
            let result = decompose(&make_handoff("T", vec![s]), &config).unwrap();
            assert_eq!(result.pieces[0].suggested_tier, AgentTier::Sonnet);

            let mut s = make_step(1, "Large");
            s.effort = Some("1 day".to_string());
            let result = decompose(&make_handoff("T", vec![s]), &config).unwrap();
            assert_eq!(result.pieces[0].suggested_tier, AgentTier::Opus);
        }

        #[test]
        fn tier_from_step_count() {
    ...
    mod rendering {
        ...
        fn render_piece_multiple_steps() {
            let piece = HandoffPiece {
                ...
                steps: vec![make_step(1, "First step"), make_step(2, "Second step")],
                estimated_effort_secs: None,
                suggested_tier: AgentTier::Sonnet,
                depends_on: Vec::new(),
            };
